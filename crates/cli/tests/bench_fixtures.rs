...
fn queries_fixture_parses_with_schema() {
    ...
    for q in queries {
        ...
        assert!(
            rel.get("source_ids").map(serde_json::Value::is_array).unwrap_or(false),
            "relevant.source_ids must be an array"
        );
        assert!(
            rel.get("chunk_ids").map(serde_json::Value::is_array).unwrap_or(false),
...
fn lens_turns_fixture_parses() {
    ...
    let txt = std::fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("lens_turns.jsonl must be committed at {}: {e}", path.display()));
    let mut turns = 0usize;
    for line in txt.lines().filter(|l| !l.trim().is_empty()) {
        let v: serde_json::Value =
            serde_json::from_str(line).expect("each lens_turns line is valid JSON");
        assert!(
            v.get("focus_text").and_then(serde_json::Value::as_str).is_some(),
            "each lens turn needs a `focus_text` string"
        );
        turns += 1;
    }
    assert!(turns >= 4, "expected several lens turns for a rotation signal, got {turns}");
}
